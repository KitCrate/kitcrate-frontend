...
- **This app's escrow token label depends on a live RPC read.** `NEXT_PUBLIC_TOKEN_CONTRACT_ID`'s real symbol is fetched once per server instance and memoized (`apps/web/lib/token.ts`); if that read fails on a cold start, the label falls back to the generic "tokens" until the instance recycles, rather than retrying. Amounts and contract behavior are never affected either way — only the displayed unit label.
